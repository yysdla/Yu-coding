"""Reserved internal model gateway provider. Live calls disabled."""
...
class InternalProvider:
    ...
    def __init__(
        self,
        *,
        model_name: str = "internal-reserved",
    ...
    async def complete(self, prompt: ContextPrompt) -> ProviderResult:
        ...
        return ProviderResult(
            ...
            live=False,
            notes=(
                "Internal provider reserved; no network call",
                f"model_name={self._model_name}",
                f"endpoint_configured={bool(self._endpoint)}",
            ),
        )
